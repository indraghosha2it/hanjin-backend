...
const { sendEmail, sendConsolidationStatusEmail } = require('../utils/emailService');
...
exports.getConsolidations = async (req, res) => {
    try {
        ...
        const consolidations = await Consolidation.find(query)
            .populate({
                path: 'shipments',
                select: 'trackingNumber status sender receiver',
                limit: 1 // Only get first shipment for sender/receiver info
            })
        ...
        // ✅ Add sender and receiver info to each consolidation
        const enrichedConsolidations = consolidations.map(cons => {
            const consObj = cons.toObject();
            
            if (cons.shipments && cons.shipments.length > 0) {
                const firstShipment = cons.shipments[0];
                
                // Add sender info
                if (firstShipment.sender) {
                    consObj.senderName = firstShipment.sender.name || '';
                    consObj.senderEmail = firstShipment.sender.email || '';
                    consObj.senderPhone = firstShipment.sender.phone || '';
                    consObj.senderCompany = firstShipment.sender.companyName || '';
                }
                
                // Add receiver info
                if (firstShipment.receiver) {
                    consObj.receiverName = firstShipment.receiver.name || '';
                    consObj.receiverEmail = firstShipment.receiver.email || '';
                    consObj.receiverPhone = firstShipment.receiver.phone || '';
                    consObj.receiverCompany = firstShipment.receiver.companyName || '';
                }
            }
...
exports.getConsolidationById = async (req, res) => {
    try {
        ...
        // ✅ Extract sender and receiver info from first shipment
        let senderInfo = {};
        let receiverInfo = {};
        ...
        if (consolidation.shipments && consolidation.shipments.length > 0) {
            ...
            if (firstShipment.sender) {
                senderInfo = {
                    ...
                    senderPhone: firstShipment.sender.phone || '',
                    senderCompany: firstShipment.sender.companyName || '',
                    senderAddress: firstShipment.sender.address?.addressLine1 || '',
                    senderCity: firstShipment.sender.address?.city || '',
                    senderCountry: firstShipment.sender.address?.country || ''
                };
            }
            ...
            if (firstShipment.receiver) {
                receiverInfo = {
                    ...
                    receiverPhone: firstShipment.receiver.phone || '',
                    receiverCompany: firstShipment.receiver.companyName || '',
                    receiverAddress: firstShipment.receiver.address?.addressLine1 || '',
                    receiverCity: firstShipment.receiver.address?.city || '',
                    receiverCountry: firstShipment.receiver.address?.country || ''
                };
            }
        }
...
exports.updateConsolidationStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const { status, actualDeparture, actualArrival, location, notes, forceUpdate, resumeTo, timestamp } = req.body;
    ...
    const getAllowedTransitions = (currentStatus) => {
      const transitions = {
        'draft': ['in_progress', 'pending', 'ready_for_dispatch', 'cancelled', 'on_hold'],
        'in_progress': ['consolidated', 'ready_for_dispatch', 'cancelled', 'on_hold'],
        'consolidated': ['ready_for_dispatch', 'pending'],
        'ready_for_dispatch': ['loaded', 'cancelled', 'on_hold'],
        'pending': ['received_at_warehouse', 'picked_up_from_warehouse', 'loaded_into_container', 'cancelled', 'on_hold'],
        'received_at_warehouse': ['picked_up_from_warehouse', 'cancelled', 'on_hold'],
        'picked_up_from_warehouse': ['loaded_into_container', 'cancelled', 'on_hold'],
        'loaded': ['dispatched', 'loaded_into_container', 'cancelled', 'on_hold'],
        'loaded_into_container': ['container_sealed', 'cancelled', 'on_hold'],
        'container_sealed': ['departed_port_of_origin', 'cancelled', 'on_hold'],
        'dispatched': ['in_transit', 'in_transit_sea_freight', 'cancelled'],
        'departed_port_of_origin': ['in_transit_sea_freight', 'cancelled'],
        'in_transit': ['arrived', 'arrived_at_destination_port', 'cancelled'],
        'in_transit_sea_freight': ['arrived_at_destination_port', 'cancelled'],
        'arrived': ['under_customs_clearance', 'customs_cleared', 'cancelled'],
        'arrived_at_destination_port': ['under_customs_clearance', 'customs_cleared', 'cancelled'],
        'under_customs_clearance': ['customs_cleared', 'cancelled'],
        'under_customs_cleared': ['customs_cleared', 'cancelled'],
        'customs_cleared': ['unloaded_from_vessel', 'out_for_delivery', 'cancelled'],
        'unloaded_from_vessel': ['out_for_delivery', 'cancelled'],
    ...
    const isForceUpdate = forceUpdate === true || forceUpdate === 'true' || forceUpdate === 1 || forceUpdate === '1';
    ...
    const getShipmentStatus = (consolStatus) => {
      const statusMap = {
        ...
        'picked_up_from_warehouse': 'picked_up_from_warehouse',
        ...
        'in_transit_sea_freight': 'in_transit_sea_freight',
    ...
    const getDescriptionForStatus = (stat, cons) => {
      const descMap = {
        ...
        'in_transit': '',
        'ready_for_dispatch': '',
        'loaded': 'Container Sealed'
      };
      return descMap[stat] || '';
    };

    const restoreStatus = status === 'in_progress' && oldStatus === 'on_hold'
      ? (resumeTo || consolidation.lastActiveStatus || 'in_progress')
      : status;

    const shipmentNewStatus = getShipmentStatus(restoreStatus);
    ...
    for (const shipment of consolidation.shipments) {
      ...
      let milestoneLocation = location || getLocationForStatus(restoreStatus, consolidation);
      let milestoneDescription = getDescriptionForStatus(restoreStatus, consolidation);
      const shouldAttachContainerDetails =
        restoreStatus === 'loaded' ||
        shipmentNewStatus === 'loaded_in_container' ||
        shipmentNewStatus === 'container_sealed';
      const milestoneMetadata = shouldAttachContainerDetails
        ? {
            containerNumber: consolidation.containerNumber || null,
            sealNumber: consolidation.sealNumber || null
          }
        : {};
      ...
      switch(restoreStatus) {
        case 'dispatched':
          shipment.transport.actualDeparture = actualDeparture ? new Date(actualDeparture) : eventTimestamp;
        ...
        case 'in_transit':
          ...
          break;
        case 'arrived':
          shipment.transport.actualArrival = actualArrival ? new Date(actualArrival) : eventTimestamp;
          shipment.transport.currentLocation = {
            location: milestoneLocation,
            status: 'arrived',
            timestamp: eventTimestamp
        ...
        case 'completed':
          ...
          shipment.dates.delivered = eventTimestamp;
          shipment.transport.currentLocation = {
            location: milestoneLocation,
            status: 'delivered',
            timestamp: eventTimestamp
    ...
    consolidation.status = status === 'in_progress' && oldStatus === 'on_hold' ? restoreStatus : status;
    consolidation.updatedBy = req.user._id;
    ...
    if (consolidation.status === 'loaded' || consolidation.status === 'dispatched') {
    ...
    consolidation.timeline.push({
      status: consolidation.status,
      timestamp: eventTimestamp,
      description: notes || `Consolidation status changed from ${oldStatus} to ${consolidation.status}`,
      updatedBy: req.user._id
    });

    await consolidation.save();

    console.log(`✅ Consolidation ${consolidation.consolidationNumber} updated to ${consolidation.status}`);
    console.log(`✅ ${updatedShipments.length} shipments updated to ${shipmentNewStatus}`);

    // ========== SEND STATUS UPDATE EMAILS ==========
    try {
      // Fetch shipments with sender/receiver info for email
      const shipmentsForEmail = await Shipment.find({ _id: { $in: consolidation.shipments } })
        .select('trackingNumber sender receiver')
        .lean();
      ...
      // Don't throw error - we still want to return successful status update even if email fails
    }

    res.status(200).json({
      success: true,
      message: `Consolidation status updated to ${consolidation.status}`,
      data: {
        consolidation,
...
exports.updateShipmentInConsolidation = async (req, res) => {
  try {
    ...
    const shipmentExists = consolidation.shipments.some(
      s => s && s._id && s._id.toString() === shipmentId
    );
