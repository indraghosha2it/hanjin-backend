...
exports.receiveShipment = async (req, res) => { 
    try {
        ...
        if (!shipmentId || !normalizedLocation || !Array.isArray(packages) || packages.length === 0) {
        ...
        if (receipt) {
            ...
            receipt.receivingCondition = condition || 'Good';
            receipt.status = condition === 'Damaged' || condition === 'Partial' ? 'damaged_report' : 'received';
            ...
            receipt = new WarehouseReceipt({
                ...
                receivingCondition: condition || 'Good',
                status: condition === 'Damaged' || condition === 'Partial' ? 'damaged_report' : 'received',
                remarks: notes || '',
                storageLocation,
                createdBy: req.user._id,
                createdAt: new Date()
        ...
        shipment.storageLocation = normalizedLocation;
...
exports.completeConsolidation = async (req, res) => {
    try {
        const { id } = req.params;
        const { containerNumber, sealNumber, documents, containerAssignments } = req.body;
        ...
        // Normalize assignments map for quick lookup (if provided)
        // Expected format: [{ shipmentId, containerNumber, sealNumber }, ...]
        const assignmentMap = {};
        if (Array.isArray(containerAssignments)) {
            for (const a of containerAssignments) {
                if (!a) continue;
                const sid = (a.shipmentId || a.shipment || a._id || a.id || '').toString();
                if (!sid) continue;
                assignmentMap[sid] = {
                    containerNumber: a.containerNumber || a.container || null,
                    sealNumber: a.sealNumber || a.seal || null
                };
            }
        }
        ...
        for (const shipmentId of consolidation.shipments) {
            ...
            if (shipment) {
                ...
                // If there's an explicit assignment for this shipment, persist it.
                const assigned = assignmentMap[shipment._id.toString()];
                if (assigned && (assigned.containerNumber || assigned.sealNumber)) {
                    shipment.containers = shipment.containers || [];
                    // Replace or append a single assigned container entry
                    shipment.containers.push({
                        containerNumber: assigned.containerNumber || '',
                        sealNumber: assigned.sealNumber || ''
                    });
                } else if (containerNumber) {
                    // If no per-shipment containers exist, set consolidation-level container
                    // but do not overwrite existing per-shipment containers
                    if (!Array.isArray(shipment.containers) || shipment.containers.length === 0) {
                        shipment.containers = [{
                ...
                shipment.milestones.push({
                    status: 'consolidated',
                    location: 'Warehouse',
                    description: `Consolidated into container ${assignmentMap[shipment._id.toString()]?.containerNumber || consolidation.containerNumber || 'N/A'}`,
                    updatedBy: req.user._id,
                    timestamp: new Date()
                });
            ...
            if (booking && booking.customer) {
                sendEmail({
                    ...
                    data: {
                        ...
                        trackingUrl: `${(process.env.FRONTEND_URL || process.env.CLIENT_URL || process.env.NEXT_PUBLIC_FRONTEND_URL || '').replace(/\/$/, '')}/tracking-number?tracking=${encodeURIComponent(shipment.trackingNumber)}`
                    }
...
exports.inspectShipment = async (req, res) => {
    try {
        ...
        receipt.inspectionHistory = receipt.inspectionHistory || [];
        receipt.inspectionHistory.push({
            conductedBy: req.user._id,
            conductedAt: new Date(),
            condition: inspectionData.condition,
            findings: inspectionData.findings || '',
        ...
        if (shipment) {
            ...
            const validStatuses = [
                ...
                'return_requested', 'return_approved', 'return_rejected', 'return_initiated', 'return_completed'
            ];
