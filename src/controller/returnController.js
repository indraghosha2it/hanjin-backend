...
/**
 * @desc    Customer Cancel Return by ReturnRequest ID
 * @route   PUT /api/v1/shipments/return-requests/:returnRequestId/cancel
 * @access  Private (Customer only)
 */
exports.cancelReturnRequest = async (req, res) => {
    try {
        ...
        // Find shipment by embedded returnRequest id in either model
        let shipment = await NewShipment.findOne({ 'returnRequest._id': returnRequestId }).populate('customerId', 'email firstName lastName');
        let model = 'NewShipment';
        if (!shipment) {
            shipment = await Shipment.findOne({ 'returnRequest._id': returnRequestId }).populate('customerId', 'email firstName lastName');
            model = 'Shipment';
        }
        ...
        // Only the requesting customer or admin can cancel
        const requesterId = shipment.returnRequest?.requestedBy && String(shipment.returnRequest.requestedBy);
        const userId = String(req.user._id);
        if (requesterId && requesterId !== userId && !(req.user.role && ['admin','super_admin'].includes(req.user.role))) {
            return res.status(403).json({ success: false, message: 'Not authorized to cancel this return request' });
        }

        if (!shipment.returnRequest || shipment.returnRequest.status === 'none' || shipment.returnRequest.status === 'rejected_by_customer') {
            return res.status(400).json({ success: false, message: 'No active return request to cancel' });
        }

        // Mark as cancelled by customer
        shipment.returnRequest.status = 'rejected_by_customer';
        shipment.returnRequest.customerRejectionReason = reason || 'Cancelled by customer';
        shipment.returnRequest.customerRejectedAt = new Date();

        // Add timeline entry
        addTimelineEntry(shipment, 'return_cancelled', `Return cancelled by customer. ${reason || ''}`, null, req.user._id);
        ...
        try {
            ...
            if (adminEmails.length > 0) {
                await sendEmail({
                    to: adminEmails,
                    subject: `🛑 Return Request Cancelled - ${shipment.trackingNumber}`,
                    html: `
                        <h3>Return Request Cancelled</h3>
                        <p>Shipment: ${shipment.shipmentNumber} (${shipment.trackingNumber})</p>
                        <p>Cancelled By: ${req.user.firstName || req.user.email || 'Customer'}</p>
                        <p>Reason: ${reason || 'Not provided'}</p>
                        <a href="${process.env.FRONTEND_URL}/admin/return-requests">Open Return Requests</a>
                    `
                }).catch(err => console.log('Admin notification error:', err.message));
            }
        } catch (notifyErr) {
            console.log('Notify admins error:', notifyErr.message);
        }

        res.status(200).json({ success: true, message: 'Return request cancelled', data: shipment.returnRequest });
