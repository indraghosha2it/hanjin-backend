...
const registerCustomerAndSendOTP = async (req, res) => {
  try {
    ...
    // Send OTP via Email
    const emailResult = await sendRegistrationOTPEmail(emailLower, otp, firstName);
    ...
    if (existingUser) {
      Object.assign(existingUser, pendingUserData);
      await existingUser.save();
    } else {
      await UserModel.create(pendingUserData);
    }
...
const verifyCustomerOTP = async (req, res) => {
  try {
    ...
    // Read pending registration from persistent storage
    ...
    if (!pendingUser.registrationOTP || !pendingUser.registrationOTPExpires) {
      return res.status(400).json({
        success: false,
        message: "Registration session expired or invalid. Please register again."
      });
    ...
    // Check OTP
    if (pendingUser.registrationOTP !== otp) {
    ...
    // Check OTP expiry
    if (new Date() > new Date(pendingUser.registrationOTPExpires)) {
      return res.status(400).json({
        success: false,
    ...
    pendingUser.registrationOTP = undefined;
    pendingUser.registrationOTPExpires = undefined;
    pendingUser.otpAttempts = 0;
    pendingUser.customerStatus = 'Active';
    pendingUser.customerSince = new Date();
    ...
    try {
      await sendWelcomeEmail(emailLower, pendingUser.firstName);
    } catch (emailError) {
    ...
    delete userData.registrationOTPExpires;
...
const resendOTP = async (req, res) => {
  try {
    ...
    const lastSent = pendingUser.updatedAt || pendingUser.createdAt;
    ...
    pendingUser.registrationOTPExpires = otpExpiry;
    pendingUser.otpAttempts = 0;
    await pendingUser.save();

    // Send OTP via Email
    const emailResult = await sendRegistrationOTPEmail(emailLower, otp, pendingUser.firstName);
...
// ==================== CREATE/GET CUSTOMER (Admin Only) ====================

const createOrGetCustomerByAdmin = async (req, res) => {
  try {
    const {
      firstName,
      lastName,
    ...
    const requester = await UserModel.findById(req.user.userId);
    if (!requester || (requester.role !== 'admin' && requester.role !== 'warehouse')) {
      return res.status(403).json({
        success: false,
        message: 'Access denied. Admin or Warehouse only.'
      });
    }
    ...
    const safeFirstName = (firstName || 'Customer').trim();
    ...
    const customer = new UserModel({
      ...
      businessType: businessType || 'Trader',
      originCountries: originCountries || ['China', 'Thailand'],
      destinationMarkets: destinationMarkets || ['USA'],
    ...
    return res.status(500).json({
      success: false,
      message: 'Failed to create or fetch customer',
      error: process.env.NODE_ENV === 'development' ? error.message : 'Internal server error'
    });
  }
};
