    ...
    try {
        ...
        let shipment = await Shipment.findOne({ trackingNumber })
            ...
            .populate({
                ...
                select: 'consolidationNumber containerNumber containerType sealNumber status timeline originWarehouse destinationPort shipments'
            })
            .lean();
        ...
        if (shipment.consolidationId && shipment.consolidationId.timeline) {
            ...
            const consolidationStatusMap = {
                ...
                'ready_for_dispatch': 'received_at_warehouse',
                ...
                'customs_clearance': 'under_customs_clearance',
            ...
            const consolidationEvents = shipment.consolidationId.timeline
                ...
                .map(event => {
                    ...
                    return {
                        ...
                        metadata: {
                            containerNumber: shipment.consolidationId?.containerNumber || null,
                            sealNumber: shipment.consolidationId?.sealNumber || null
                        },
        ...
        // 5. টাইমলাইন সাজান - প্রথমে প্রাথমিক ইভেন্ট, তারপর তারিখ অনুযায়ী (নতুন প্রথমে)
        timeline.sort((a, b) => {
            // আগে প্রাথমিক ইভেন্ট দেখান
            if (a.isPrimary && !b.isPrimary) return -1;
            if (!a.isPrimary && b.isPrimary) return 1;
            // তারপর তারিখ অনুযায়ী (নতুন প্রথমে - সবচেয়ে পুরাতন শেষে)
        ...
        // Resolve container/seal per shipment first, then fall back to consolidation-level values.
        const consolidationData = shipment.consolidationId || null;
        let resolvedContainerNumber = null;
        let resolvedSealNumber = null;
        const shipmentContainers = Array.isArray(shipment.containers)
            ? shipment.containers
                .map((container) => ({
        ...
        if (consolidationData?.shipments && Array.isArray(consolidationData.shipments)) {
            const matchedEntry = consolidationData.shipments.find((entry) => {
                ...
                const linkedShipmentId = entry.shipmentId || entry._id || entry.id || entry;
                return String(linkedShipmentId) === String(shipment._id);
            });
            ...
            if (matchedEntry && typeof matchedEntry === 'object') {
                resolvedContainerNumber =
                    ...
                    matchedEntry.containerNumbers?.[0] ||
                    null;
                ...
                resolvedSealNumber =
                    ...
                    matchedEntry.sealNo ||
                    matchedEntry.seal ||
                    matchedEntry.sealNumbers?.[0] ||
                    null;

                if (!shipmentContainers.length) {
                    const matchedContainers = Array.isArray(matchedEntry.containerNumbers) || Array.isArray(matchedEntry.sealNumbers)
                        ? []
                        : [];

                    if (matchedEntry.containerNumbers || matchedEntry.sealNumbers) {
                        const matchedContainerNumbers = Array.isArray(matchedEntry.containerNumbers)
                            ? matchedEntry.containerNumbers
                            : `${matchedEntry.containerNumber || matchedEntry.containerNo || matchedEntry.container || ''}`.split(',').map((item) => item.trim()).filter(Boolean);
                        const matchedSealNumbers = Array.isArray(matchedEntry.sealNumbers)
                            ? matchedEntry.sealNumbers
                            : `${matchedEntry.sealNumber || matchedEntry.sealNo || matchedEntry.seal || ''}`.split(',').map((item) => item.trim()).filter(Boolean);

                        const pairLength = Math.max(matchedContainerNumbers.length, matchedSealNumbers.length);
                        for (let index = 0; index < pairLength; index += 1) {
                            matchedContainers.push({
                                containerNumber: matchedContainerNumbers[index] || '',
                                sealNumber: matchedSealNumbers[index] || ''
                            });
                        }

                        shipmentContainers.push(...matchedContainers.filter((container) => container.containerNumber || container.sealNumber));
                    }
                }
            }
        }
        ...
        if (!resolvedContainerNumber) {
            resolvedContainerNumber = consolidationData?.containerNumber || null;
        }

        if (!resolvedSealNumber) {
            resolvedSealNumber = consolidationData?.sealNumber || null;
        }
        ...
        const publicTrackingInfo = {
            ...
            // Convenience fields for clients that read top-level values
