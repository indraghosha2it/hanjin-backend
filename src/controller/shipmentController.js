...
exports.createShipment = async (req, res) => {
  try {
    ...
    let trackingNumber;
    let shipmentNumber;
        const normalizedProvidedTrackingNumber = frontendTrackingNumber?.trim().toUpperCase();
    
    // Use tracking number from frontend if provided, otherwise generate new
        if (normalizedProvidedTrackingNumber) {
            // Check if tracking number already exists anywhere that the dashboard reads from
            const [existingShipment, existingBooking, existingNewShipment] = await Promise.all([
                Shipment.findOne({ trackingNumber: normalizedProvidedTrackingNumber }),
                Booking.findOne({ trackingNumber: normalizedProvidedTrackingNumber }),
                NewShipment.findOne({ trackingNumber: normalizedProvidedTrackingNumber })
            ]);

            if (existingShipment || existingBooking || existingNewShipment) {
        return res.status(400).json({
          success: false,
                    message: `Tracking number ${normalizedProvidedTrackingNumber} already exists`
        });
      }
            trackingNumber = normalizedProvidedTrackingNumber;
...
exports.updateShipmentTrackingNumber = async (req, res) => {
    try {
        ...
        const [existingShipment, existingBooking, existingNewShipment] = await Promise.all([
            Shipment.findOne({ trackingNumber: normalizedTrackingNumber, _id: { $ne: id } }),
            Booking.findOne({ trackingNumber: normalizedTrackingNumber }),
            NewShipment.findOne({ trackingNumber: normalizedTrackingNumber })
        ]);
